...
module.exports = async function (req, res, next) {
    try {
        let users = await sequelize.query(`SELECT name, id FROM users WHERE name LIKE :search AND NOT id = :id`, {replacements: {search : `%${req.query.value}%`, id: req.body.userId}, type: sequelize.QueryTypes.SELECT});
        if (users.length > 0) {
